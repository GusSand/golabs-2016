...
func Map(value string) *list.List {
	...
	var wordList = strings.FieldsFunc(value, wordFinder)

	var wordCountMapList = list.New()

	for _, word := range wordList {
		wordCountMapList.PushBack(mapreduce.KeyValue{ word, "1" })
	}

	return wordCountMapList
}
...
func Reduce(key string, values *list.List) string {
	var listLength = values.Len()
	var stringifiedListLength = strconv.Itoa(listLength)
	return stringifiedListLength
}
